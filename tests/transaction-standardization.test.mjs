...
function testNullHandling() {
  ...
  // Test 3: Empty string for signer/signature
  // Note: We always set signer/signature explicitly, so empty strings should never occur in practice
  // But if they did, with explicit null check: rest.signer !== null && rest.signer !== undefined
  // Empty string "" is neither null nor undefined, so it would be String("").toLowerCase() = ""
  // This is acceptable since we never use empty strings in practice
